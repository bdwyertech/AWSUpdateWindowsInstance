...
Function Install-AwsUwiEC2Config
{
    <#
        .SYNOPSIS
        Installs EC2Config.  If already installed, it will upgrade EC2Config to the latest version.
        ...
        .PARAMETER Region
        The AWS region that your instance is in that is executing this script.  e.g. 'us-east-1'
        ...
        .EXAMPLE
        Install-AwsUwiEC2Config -Region 'us-east-1' -Logger $Logger

        .EXAMPLE
        Install-AwsUwiEC2Config -Region 'us-east-1' -WorkingDirectory 'C:\Temp' -Logger $Logger

        .EXAMPLE
        Install-AwsUwiEC2Config -Region 'us-east-1' -Id 'abc123' -WorkingDirectory 'C:\Temp' -Logger $Logger
    #>
    ...
    Begin
    {
        ...
        $functionName = "Install-AwsUwiEC2Config"
    ...
    Process
    {
        ...
        {
            $Logger.Error("Unsupported operating system.  EC2Config is supported on Windows Server 2012 R2 and below.")
            exit -1
        }
        ...
        {         
            ...
            {
                $Logger.Info("Windows requires a reboot.  Sending reboot request to SSM Agent.")
                exit 3010
            }
            ...
            {
                $url = "https://s3.$($Region).amazonaws.com.cn/aws-ssm-$($Region)/latest/EC2ConfigSetup.zip"
            } 
            elseif($Region.StartsWith('us-gov-')) 
            {
                $url = "https://s3-$($Region).amazonaws.com/aws-ssm-$($Region)/latest/EC2ConfigSetup.zip"
            } 
            elseif($Region -eq 'us-east-1') 
            {
                $url = "https://s3.amazonaws.com/aws-ssm-$($Region)/latest/EC2ConfigSetup.zip"
            } 
            else 
            {
                $url = "https://aws-ssm-$($Region).s3.amazonaws.com/latest/EC2ConfigSetup.zip"
            }

            try
            {
                (New-Object System.Net.WebClient).DownloadFile($url, $zipfile)
                $Logger.Info("Successfully downloaded EC2Config.")
            }
            catch
            {
                $Logger.Error("Download of EC2Config failed with error: $($_)")
                exit -1
            }
            ...
            {
                $Logger.Warn("Unblocking of EC2Config zip file failed with error: $($_)")
            }
            ...
            try
            {
                ...
                $Logger.Info("Successfully unzipped EC2Config.")
            }
            catch
            {
                $Logger.Error("Unzipping of EC2Config failed with error: $($_)")
                exit -1	
            }
            ...
            if(Test-Path $ec2install)
            {
                ...
                try
                {
                    ...
                    <# 
                        system.version is a 4 digit version
                        ec2install.exe uses a 4 digit e.g. 4.8.1676.0
                        ec2config.exe only uses a 3 digit version. e.g. 4.8.1676.  This results in the version revision digit to be a -1
                        need to handle ec2config revision digit to ensure comparison is performed correctly
                    #>            
                ...
                {
                     $Logger.Error("Determining version of EC2Config failed with error: $($_)")
                     exit -1
                }
                ...
                {
                    $Logger.Info("Upgrade not required. EC2Config is already up-to-date with version $($downloadedVersion).")
                    $shouldInstall = $false     
                }
            }
            else
            {
                $Logger.Error("EC2Install.exe not found at $($ec2install).")
                exit -1	
            }
            ...
            if($shouldInstall)
            {
                try
                {
                    ...
                    $Logger.Info("Successfully installed EC2Config version $($afterInstallVersion).")            
                }
                catch
                {
                    $Logger.Error("Installation of EC2Config failed with error: $($_)")
                    exit -1
                }
        ...
        {
            $Logger.Warn("Cleanup failed with error: $($_)")
        }
    }
    End
    {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC