...
Function Start-DetectLaunchAgent
{
    <#
        .SYNOPSIS
        Detects which launch agent is installed.  Returns an object with the launch agent name and version.
                
        .DESCRIPTION
        Detects which launch agent is installed.  Returns an object with the launch agent name and version.

        .EXAMPLE
        Start-DetectLaunchAgent
    #>
    ...
    Process
    {
        ...
        if($ec2LaunchVersion -and $ec2ConfigVersion)
        {
            throw [AWSUpdateWindowsInstanceException.LaunchAgentDetectionException] "Both EC2Config and EC2Launch have been detected; only one should be installed."
        }

        if((-not $ec2LaunchVersion) -and (-not $ec2ConfigVersion))
        {
            throw [AWSUpdateWindowsInstanceException.LaunchAgentNotDetectedException] "Neither EC2Config or EC2Launch have been detected. EC2Launch or EC2Config must be installed in order to perform sysprep."
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBPJ80GlerAAj8w
# VPAlsF+B9tepr0pDNGW2hmmNhjIt2KCCDJwwggXYMIIEwKADAgECAhABVznfx2xi
...
# INrHFBfyyU10C3JPgpFivaIYpziAju6FiRCoikqgnXNhMA0GCSqGSIb3DQEBAQUA
# BIIBAHPGsKjt9tBNyoGYO5wQ/TAhDIeAlu3nFNeoNg49xw/gw2iq8G+PbWXg13d8
# GwWGDMLv8HhHuAeIbYu2wEpYUG28JcYaOJIbxnJ+wXDRzbehlNXD58mBK4K1UC1z
# o/hOOwnEg4XgpxSybAluucwdHHXyiKIaIyoi89UiKcZy+EkzaUmEfeNAKciapfAJ
# Gg/fBf4bfo1vfiQyZSoXkNqASawmfZWIuxJKt7eT6y51b/2XkJklUOox92V/Lvq9
# ti7qVkFv0MGB1l+DAx0HD2Owz7+lrLHQjvDQdw5FcAHVn48d83bbRImjjcVB68uX
# rH+wl5GpSaQRweCNsImdY52hXhOhgg7IMIIOxAYKKwYBBAGCNwMDATGCDrQwgg6w
# BgkqhkiG9w0BBwKggg6hMIIOnQIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb3
# DQEJEAEEoGgEZjBkAgEBBglghkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgntjc
# RzkZkxqAc8qNdtdhDo5Eqsnx0vOSpBDZq4JTro8CEFFbEmy2Hz9ZqopRKkgAIdsY
...
# oqwaJiFWI5bQIFqHNgEs0dLk+nyD1DxNaVX+rI8fH0MwKwYLKoZIhvcNAQkQAgwx
# HDAaMBgwFgQUQAGRR1yYiR3roQSvRwkbXrbUy8swDQYJKoZIhvcNAQEBBQAEggEA
# Qzp72TIxraLsC1Ux023E7BojHpeUe7qNfNe7uvs3Q+Iz0J1AciRdlvEbpO49ie69
# rujmfD/nMD/VDEVwYBu66MLei3IVbWdKspbTaCzUFgPHZOveamqSytRl/M+9dykw
# IxirJwxVkZreYX1wmSctCezh/xOMrfn7wYe12ouHOHhV2z3ZE2xTEoQh1lM5mnVC
# BRDqqohLPzR9jsaCes74Zv0OaIbeEcR8dmminjwOc+M41AUi2utQVivd35Azejnu
# dYbmMY1hpP6OpzRfc/P+NvhMqLDv7oj11MbrqV3et5+b+sXuqxvFKXMX67k9tG0F
# rbcGBx4orus5RCKAM/x2fw==
